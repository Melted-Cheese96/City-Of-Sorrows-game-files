...
import newcombatsystem as newComb
import PoliceMission as police
import Merchants as merchant
    ...
    while True:
        ...
        if choice.lower() == 'look for the thugs':
            ...
            if choice2.lower() == 'left':  # Finish!
                ...
                sleep(2)
                print("Do you go to the 'tavern', 'park' or 'shopping centre'?")
                townsquare = input()
                if townsquare.lower() == 'tavern':
                    Tavern()
                elif townsqquare.lower() == 'park':
                    pass
                elif townsquare.lower() == 'shopping centre':
                    pass

            else:
                print('Wrong entry, try again')
        elif choice.lower() == 'go home':  # The rest of the code for this section is in the GunInvestigation function
            guni.GunInvestigation()
...
def TavernQuestFight():  # This function gets called once you leave the tavern. NOTE: THIS FUNCTION IS FINISHED
    ...
    if fight.lower() == 'fight':  # Continue the rest
        newComb.fightTest(PlayerName, thug)
        print('Buffer')
        police.PoliceMissionPt1(PlayerName)
    ...
    if ballquestsewerspt1 == 'follow the cries':
        ...
        while True:
            ...
            if ballquestpt2 == 'try to break rope':
                ...
                Boy = Child()
                print('Child\'s health ' + str(Boy.health))
                sleep(2)
...
def RandomMoney(player):
    money = ['While walking on the road you see some money on the ground', '']
    fate = random.choice(money)
    print(fate)
    if fate == money[0]:
        AddMoney(player, 50)
...
def Intro():
    ...
    print('QUICK TIP: Fighting in this game is a little challenging, do not get angry if you lose a lot')
    print('Welcome to the City Of Sorrows! Do you want to get the backstory?')
    backstory = input()
    if backstory == 'yes':
        Backstory('City of sorrows lore', 'yes')
        print('You get 50 gold as a starting gift')
        AddMoney(PlayerName, 50)
        print(PlayerName.inventory)
        main()
    elif backstory == 'no':
        main()
